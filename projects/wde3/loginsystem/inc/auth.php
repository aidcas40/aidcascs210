...
* @return bool
*/
function insert_course(string $crs_id, string $crs_title, int $crs_credits, string $crs_program): bool
{
    $sql = 'INSERT INTO course(crs_id, crs_title, crs_credits, crs_program)
            VALUES(:id, :title, :credits, :program)';

    $statement = db()->prepare($sql);

    $statement->bindValue(':id', $crs_id);
